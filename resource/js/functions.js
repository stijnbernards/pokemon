...
var maps = [
    ...
    "maps/towns/rustboro_city/main.js",             //22
...
var pokemonCore = {
                ...
                switch (e.which) {
                    ...
                    case 48:
                        if (pokemonCore.maps.collision) {
                            ...
                            pokemonCore.maps.collision = true;
                            pokemonCore.maps.addNumber = 1;
                        }
                        break;
                    case 49:
            ...
            for (var i = seen[0]; i <= seen[seen.length - 1]; i++) {
                ...
                if (seen.indexOf(pokemon.nN) > -1) {
                    ...
                    if (pokemonCore.fileSystem) {
                        $(".pokemon-images").append('<div class="pkdexpkmn"></div>');
                        pokemonCore.fileStorage.retrieveFileBg("pkmn" + pokemon.nN + ".txt", $(".pokemon-images .pkdexpkmn"));
                    } else {
                                                ...
                                                pokemonCore.utils.writer(0, function () {
                                                    setTimeout(function () {
                                                        ...
                                                        pokemonCore.utils.writer(0, function () {
                                                            ...
                                                            if (pokemonCore.fileSystem) {
                                                                pokemonCore.fileStorage.retrieveFileBg("pkmn" + pokemonCore.gameChar.pokemon.nN + ".txt", $(".ally-pokemon"));
                                                            } else {
    ...
    npc: function (pA, nm, dia, aDC, bC, txt, start, battle, shouldTurn, beforeFight, afterFight, dir, leader) {
        ...
        function moveNpc(x, y) {
            ...
            coords[1] += y;
            $(".npc-" + nm).css({
                "top": ((coords[1] - 1) * 64 + pokemonCore.maps.ignore[0]) + "px",
                "left": ((coords[0] - 1) * 64 + pokemonCore.maps.ignore[1]) + "px"
            });
            if (x == 1)
                direction = "right";
            else if (x == -1)
        ...
        this.moveNpc = function (x, y) {
            ...
            coords[1] += y;
            $(".npc-" + nm).css({
                "top": ((coords[1] - 1) * 64 - pokemonCore.maps.ignore[1]) + "px",
                "left": ((coords[0] - 1) * 64 - pokemonCore.maps.ignore[0]) + "px"
            });
            if (x == 1)
                direction = "right";
            else if (x == -1)
        ...
        function checkPlayer(x, y) {
            ...
            function equalsArr() {
                for (var i = 0; i < collisions.length; i++) {
                    if (pokemonCore.maps.map[collisions[i][1]][collisions[i][0]] == 1)
                        break;
        ...
        startBattleScreen: function (pokemon) {
            ...
            setTimeout(function () {
                setTimeout(function () {
                    ...
                    setTimeout(function () {
                        if (pokemonCore.fileSystem) {
                            pokemonCore.fileStorage.retrieveFileBg("pkmn" + pokemon.pokemon.nN + ".txt", $(".enemy-pokemon"));
                            $(".enemy-pokemon").css("background-size", "auto auto");
                        } else {
                                            ...
                                            setTimeout(function () {
                                                ...
                                                writer(0, false, function () {
                                                    ...
                                                    setTimeout(function () {
                                                        ...
                                                        $(".action-menu").text("");
                                                        if (pokemonCore.fileSystem) {
                                                            pokemonCore.fileStorage.retrieveFileBg("pkmn" + pokemonCore.gameChar.pokemon.nN + ".png", $(".ally-pokemon"));
                                                        } else {
                                                        ...
                                                        writer(0, false, function () {
                                                            ...
                                                            setTimeout(function () {
                                                                ...
                                                                setTimeout(function () {
                                                                    if (pokemonCore.fileSystem) {
                                                                        pokemonCore.fileStorage.retrieveFileBg("pkmn" + pokemonCore.battle.trainerNpc.battle.pokemon[alivePokemon].pokemon.pokemon.nN + ".txt", $(".enemy-pokemon"));
                                                                        $(".enemy-pokemon").css("background-size", "auto auto");
                                                                    } else {
        ...
        instantiateMoves: function (pokemon) {
            for (var i = 0; i < pokemon.pokemon.moves.length; i++) {
                ...
                if (id.constructor !== Array) {
                    $.ajax({
                        async: false,
                        url: "resource/moves/" + id.replace(" ", "") + ".js",
                        dataType: "script"
                    });
        ...
        startServer: function () {
            ...
            pokemonCore.server.socket.onopen = function () {
                ...
                $(".message-box input").bind("keydown", function(e){
                    if(e.which == 13){
                        pokemonCore.server.sendMessage($(".message-box input").val());
            ...
            pokemonCore.server.socket.onmessage = function (evt) {
                switch (evt.data.split("$")[0]) {
                    ...
                    case "3":
                        ...
                        if(data != "") {
                            for (var i = 0; i < data.split('.').length; i++) {
                                $(".message-box ul").append('<li>' + data.split('.')[i] + '</li>');
                            }
